...
class ChimeService extends GetxService {
  ...
  // Rate-limit volumeChanged events to 1/sec per attendee
  final _lastVolumeTime = <String, DateTime>{};
  ...
  void listenToNativeEvents() {
    _nativeEventSubscription = _eventChannel.receiveBroadcastStream().listen(
      (event) {
        try {
          if (event is Map) {
            handleNativeEvent(deepCastMap(event));
          }
        } catch (e) {
          AppLogger.error('Error handling native event', tag: 'CHIME', error: e);
          emitEvent(MeetingEventType.error, 'Error processing callback: $e');
        }
      },
      onError: (error) {
        AppLogger.error('Native event stream error', tag: 'CHIME', error: error);
        emitEvent(MeetingEventType.error, 'Event stream error: $error');
      },
    );
  }

  static Map<String, dynamic> deepCastMap(Map map) {
    return map.map((key, value) => MapEntry(
          key.toString(),
          value is Map ? deepCastMap(value) : value,
        ));
  }
  ...
  void handleNativeEvent(Map<String, dynamic> event) {
    ...
    switch (type) {
      case 'meetingStarted':
        ...
        emitEvent(MeetingEventType.meetingStarted, 'Meeting session started');
        break;
      ...
      case 'meetingStopped':
        ...
        resetState();
        emitEvent(MeetingEventType.meetingStopped, 'Meeting session stopped: ${data['reason'] ?? 'unknown'}');
        break;
      case 'audioSessionStarted':
        emitEvent(MeetingEventType.audioSessionStarted,
            'Audio session started (reconnecting: ${data['reconnecting'] ?? false})');
        break;
      case 'audioSessionStopped':
        emitEvent(MeetingEventType.audioSessionStopped, 'Audio session stopped');
        break;
      case 'attendeeJoined':
        ...
        emitEvent(MeetingEventType.attendeeJoined, 'Attendee joined: $externalId ($attendeeId)');
        break;
      case 'attendeeLeft':
        final attendeeId = data['attendeeId'] ?? '';
        final externalId = data['externalUserId'] ?? '';
        _lastVolumeTime.remove(attendeeId);
        if (remoteAttendeeId.value == attendeeId) {
          remoteAttendeeId.value = null;
          remoteVideoTileId.value = null;
        }
        emitEvent(MeetingEventType.attendeeLeft, 'Attendee left: $externalId ($attendeeId)');
        break;
      case 'localMute':
        isMicEnabled.value = false;
        emitEvent(MeetingEventType.localMute, 'Local audio muted');
        break;
      case 'localUnmute':
        isMicEnabled.value = true;
        emitEvent(MeetingEventType.localUnmute, 'Local audio unmuted');
        break;
      case 'remoteMute':
        emitEvent(MeetingEventType.remoteMute, 'Remote attendee muted: ${data['attendeeId']}');
        break;
      case 'remoteUnmute':
        emitEvent(MeetingEventType.remoteUnmute, 'Remote attendee unmuted: ${data['attendeeId']}');
        break;
      case 'videoTileAdded':
        ...
        emitEvent(MeetingEventType.videoTileAdded, '${isLocal ? "Local" : "Remote"} video tile added (id: $tileId)');
        break;
      case 'videoTileRemoved':
        ...
        emitEvent(
            MeetingEventType.videoTileRemoved, '${isLocal ? "Local" : "Remote"} video tile removed (id: $tileId)');
        break;
      case 'videoTilePaused':
        emitEvent(MeetingEventType.videoTilePaused, 'Video tile paused (id: ${data['tileId']})');
        break;
      case 'videoTileResumed':
        emitEvent(MeetingEventType.videoTileResumed, 'Video tile resumed (id: ${data['tileId']})');
        break;
      case 'activeSpeaker':
        emitEvent(MeetingEventType.activeSpeaker, 'Active speaker: ${data['attendeeId']}');
        break;
      case 'volumeChanged':
        final volAttendee = data['attendeeId'] as String? ?? '';
        if (volAttendee.isEmpty) break;
        final now = DateTime.now();
        final last = _lastVolumeTime[volAttendee];
        if (last == null || now.difference(last).inMilliseconds > Constants.volumeEventRateLimitMs) {
          _lastVolumeTime[volAttendee] = now;
          emitEvent(MeetingEventType.volumeChanged, 'Volume: $volAttendee -> ${data['volume']}');
        }
        break;
      case 'deviceChanged':
        emitEvent(MeetingEventType.deviceChanged, 'Device changed: ${data['device']}');
        break;
      case 'audioRouteChanged':
        emitEvent(MeetingEventType.audioRouteChanged, 'Audio route: ${data['route']}');
        break;
      case 'networkDegraded':
        networkQuality.value = 'Poor';
        emitEvent(MeetingEventType.networkDegraded, 'Network quality degraded');
        break;
      case 'reconnectAttempt':
        reconnectAttempts.value++;
        emitEvent(MeetingEventType.reconnectAttempt, 'Reconnect attempt #${reconnectAttempts.value}');
        break;
      case 'connectionRecovered':
        networkQuality.value = 'Good';
        reconnectAttempts.value = 0;
        emitEvent(MeetingEventType.connectionRecovered, 'Connection recovered');
        break;
      case 'sessionFailure':
        emitEvent(MeetingEventType.sessionFailure, 'Fatal: ${data['error'] ?? 'Session failure'}');
        break;
      default:
        emitEvent(MeetingEventType.info, 'Unknown event: $type');
    }
  }

  void emitEvent(MeetingEventType type, String message, {Map<String, dynamic>? metadata}) {
  ...
  Future<bool> startMeeting({
    ...
    try {
      ...
      // Set before invoke so attendeeJoined events that fire during the call can distinguish local from remote
      _localAttendeeId = attendee.attendeeId;
      final result = await _channel.invokeMethod('startMeeting', {
        'meeting': meeting.toJson(),
        'attendee': attendee.toJson(),
      });
      if (result != true) _localAttendeeId = null;
      return result == true;
    } on PlatformException catch (e) {
      AppLogger.error('Failed to start meeting', tag: 'CHIME', error: e);
      emitEvent(MeetingEventType.error, 'Failed to start: ${e.message}');
      return false;
    }
  }
  ...
  Future<void> switchCamera() async {
    try {
      ...
      emitEvent(MeetingEventType.deviceChanged, 'Switched to ${isUsingFrontCamera.value ? "front" : "back"} camera');
    } on PlatformException catch (e) {
  ...
  Future<void> bindVideoView(int tileId, int viewId) async {
    try {
      await _channel.invokeMethod('bindVideoView', {'tileId': tileId, 'viewId': viewId});
    } on PlatformException catch (e) {
